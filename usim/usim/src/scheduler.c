...
/* Each cycle it is possible to issue a valid command from the read or write queues
   ...
   Before issuing a command it is important to check if it is issuable. For the RD/WR queue resident commands, checking the "command_issuable" flag is necessary. To check if any of the other commands mentioned above can be issued it is important to check the appropriate flag out of cmd_precharge_issuable, cmd_all_bank_precharge_issuable, cmd_powerdown_fast_issuable, cmd_powerdown_slow_issuable, cmd_powerup_issuable, cmd_refresh_issuable
   */
